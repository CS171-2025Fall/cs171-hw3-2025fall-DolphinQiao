...
public:
   ...
   * @param Float the PDF of obtaining this sample
   * @return Vec3f The BSDF value for this sample
   */
...
/**
 * @brief A perfect refraction glass BSDF. This BSDF is non physical and only
 * for testing purposes.
 *
 * 1. The refraction is perfect, i.e., if a refraction occurs, there is no
 *    energy going to reflection.
 * 2. If there is no refraction (i.e., total internal reflection), then all
 *    energy goes to reflection.
 *
 * Note that in realistic glass BSDF, there is always both reflection and
 * refraction components when refraction happens.
 */
class PerfectRefraction final : public BSDF {
public:
  // ++ Required by ConfigurableObject
  PerfectRefraction(const Properties &props);
  // --
...
private:
  /// @brief The index of refraction (ior)
  Float eta;
};
...
RDR_REGISTER_FACTORY(BSDF, [](const Properties &props) -> BSDF * {
  ...
  if (type == "diffuse") {
    ...
    return Memory::alloc<PerfectRefraction>(props);
  } else {
